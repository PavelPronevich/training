...
namespace zadacha
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                EV += (salat1[i] as FoodStuff).GetEnergeticValue();
                Console.WriteLine("В ингридиенте {0} массы {1} содржится {2} Ккал", (salat1[i] as FoodStuff).GetName(), 
                    (salat1[i] as FoodStuff).GetMassa(), (salat1[i] as FoodStuff).GetEnergeticValue());
            ...
            for (int i = 0; i < salat1.Count; i++)
            {
                if (((salat1[i] as FoodStuff).GetEnergeticValue() >= minEV) &&
                    ((salat1[i] as FoodStuff).GetEnergeticValue() <= maxEV)) {
                        Console.WriteLine("В ингридиенте {0} содержится {1} Ккал", (salat1[i] as FoodStuff).GetName(), (salat1[i] as FoodStuff).GetEnergeticValue());
                }
            }
