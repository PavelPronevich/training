    ...
    class BDChange
    {
        ...
        public static int? GetManagerID(string managerSurname)
        {
            using (var db = new OrdersContext())
            {
                ...
                {
                    return db.Managers.FirstOrDefault(x => x.ManagerSurname == managerSurname).Id;
                }
            }
            return null;
        }
        ...
        public static int? GetCustomerID(string customerName)
        {
            using (var db = new OrdersContext())
            {
                ...
                {
                    return db.Customers.FirstOrDefault(x => x.CustomerName == customerName).Id;
                }
            }
            return null;
        }
        ...
        public static int? GetProductID(string productName)
        {
            using (var db = new OrdersContext())
            {
                ...
                {
                    return db.Products.FirstOrDefault(x => x.ProductName == productName).Id;
                }
            }
            return null;
        }
                ...
                if (GetManagerID(managerName) != null)
                {
                    ...
                    using (var db = new OrdersContext())
                    {
                        ...
                        foreach (var item in productsID)
                        {
                            ...
                            {
                                db.Products.Remove(db.Products.FirstOrDefault(x => x.Id == item));
                            }
                        }
                        db.SaveChanges();
                        foreach (var item in customersID)
                        {
                            ...
                            {
                                db.Customers.Remove(db.Customers.FirstOrDefault(x => x.Id == item));
                            }
                        }
                        db.SaveChanges();
                        if (db.Orders.Count(x => x.ManagerID == managerID) == 0)
                        {
                            db.Managers.Remove(db.Managers.FirstOrDefault(x => x.Id == managerID));
                        }
                        db.SaveChanges();
                    }
                }
